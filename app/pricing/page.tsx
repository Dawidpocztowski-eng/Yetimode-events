...
export default function PricingPage() {
  ...
  useEffect(() => {
    ...
    if (searchParams.get('payment') === 'success') {
      toast.success('Płatność zakończona! Plan aktywowany 🎉')
    }
  ...
  const handleBuy = async (planKey: PlanKey) => {
    ...
    try {
      ...
      if (data.url) {
        ...
        toast.error(data.error || 'Błąd płatności')
      }
    } catch {
      ...
      <nav className="relative z-10 border-b border-white/5 px-6 py-4">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2 font-bold text-white">
            <Sparkles size={18} className="text-violet-400" /> YetiMode
          </Link>
          {user
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 py-16">
        ...
        <div className="text-center mb-14">
          ...
          <p className="text-gray-400 text-lg max-w-xl mx-auto">
            Jednorazowa opłata — bez subskrypcji, bez niespodzianek.
          </p>
          ...
          {(Object.entries(PLANS) as [PlanKey, typeof PLANS[PlanKey]][]).map(([key, plan]) => {
            ...
            return (
              <div key={key} className={`relative rounded-2xl p-6 flex flex-col transition-all ${
                isPopular
                  ? 'bg-gradient-to-b from-violet-600/20 to-indigo-600/10 border border-violet-500/30'
                ...
                {isCurrent ? (
                  <div className="w-full text-center py-3 rounded-xl bg-green-500/15 border border-green-500/20 text-green-400 text-sm font-medium">
                    ✓ Aktywny plan
                  </div>
                ) : (
                  <button
        ...
        <div className="mt-16 max-w-2xl mx-auto">
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={q} className="bg-white/3 border border-white/8 rounded-2xl p-5">
